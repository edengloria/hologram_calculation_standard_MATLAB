%% CGH 저장 파일명 설정
filename_save = ['CGH_mat_' num2str(Filename_keyword) '_color_' num2str(lm_cnt)];

%%
d_view = 'off';
%% Multifocus 인 경우

if (length(theta_ref) ~= 1) && strcmp(propagation_type,'DSF')

    cal_cnt = 0;
    complex_sum = zeros(Ny,Nx,'single');
    for im_cnt = 1:length(image_center_)
        complex_sum_ = Double_Fresnel_transfer_GPU(fmap(:,:,lm_cnt,im_cnt), xx, yy, -virtual_center, -image_center_(im_cnt), DF_image_scale(im_cnt))*DF_intensity_scale(im_cnt)^2;
        if theta_ref(im_cnt) ~= 0
            kx0 = single(k0*sind(theta_ref(im_cnt))*cosd(phi_ref(im_cnt)));
            ky0 = single(k0*sind(theta_ref(im_cnt))*sind(phi_ref(im_cnt)));
            phase_ref = -kx0*repmat(xx,size(yy.'))-ky0*repmat(yy.',size(xx));
            complex_sum_ = complex_sum_.*exp(-j*phase_ref);
            clear phase_ref;
        end
        complex_sum = complex_sum + complex_sum_;
        clear complex_sum_;
    end
    ...
    switch calculation_type
        ...
        case 'Fourier_plane'
            complex_sum = plane_2_CGH(squeeze(fmap(:,:,lm_cnt,:)),d_view,image_center_,xx,yy,propagation_type,random_phase_switch); 
    end
    ...
    if strcmp(complex_sum,'not used') == 1
        ...
        switch ref_cond
            ...
            case 'point'
                for cnt1 = 1:1:div_num
                    ...
                    temp = bin_read(filename3,[Ny,Nx],[length(y_range) Nx],[Nx_div*(cnt1-1)+1 1]);
                    % phase_ref = -k0*sqrt((repmat(xx,size(yy(y_range).'))-point_ref(1)).^2 + (repmat(yy(y_range).',size(xx))-point_ref(2)).^2 + point_ref(3).^2);
                    temp = temp.*exp(-j*(-k0*sqrt((repmat(xx,size(yy(y_range).'))-point_ref(1)).^2 + (repmat(yy(y_range).',size(xx))-point_ref(2)).^2 + point_ref(3).^2)));
                    bin_write(temp,filename4,[Ny,Nx],[Nx_div*(cnt1-1)+1 1]);
                end
        end
        ...
        switch ref_cond
            ...
            case 'point'
                phase_ref = -k0*sqrt((repmat(xx,size(yy.'))-point_ref(1)).^2 + (repmat(yy.',size(xx))-point_ref(2)).^2 + point_ref(3).^2);
                complex_sum = complex_sum.*exp(-j*phase_ref);
                clear phase_ref;
        end
    end
