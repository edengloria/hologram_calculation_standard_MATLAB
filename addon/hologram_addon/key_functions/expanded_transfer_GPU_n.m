function Hologram = expanded_transfer_GPU_n(Background_amp, expansion_factor, theta_recon, phi_recon, theta_obs, phi_obs, z_propagation, symmetry_mode,n_material) 
%% 2019-08-21 수정점
...
if Ideal_angle_limit >= atand(Nx*px/abs(z_propagation))
    disp(['알림:  Ideal_angle_limit >= atand(Nx*px/z_propagation) 이기에 주파수 필터를 동작시킵니다!']);
    ASM_filter = 1;
    theta_limit = atand(Nx*px/abs(z_propagation));
...
%% 픽셀수가 작을 때
if expansion_factor*NN <= decision_factor % 16k 보다 맵이 작으면 분할하지 않음
%%
if theta_recon ~= 0  
