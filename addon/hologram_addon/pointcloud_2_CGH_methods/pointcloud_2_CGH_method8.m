...
%% 가까운 점으로 매핑하고, 중복되는 점은 지운다 / [빛트코인] 2021/10 RGB Intensity 정보 추가
[x_p, y_p, z_p, I_p] = mapping_and_erasing_points_2(xx,yy,zz,point_array,point_number,z_num);
...
%      uv_map = uv_map + kron(temp,uv_map_small);    
     ...
     uv_map = uv_map + I_p(z_map(p_cnt))*UU.*VV;
%      cnt = cnt + 1 ;
