%% 2020/08/18 4번 방법 최신 수정 -> 바로 주파수 도메인으로 솔루션을 구한다
global CPU_max_Nx GPU_max_Nx system_angle_limit;
...
%    theta_limit_max = system_angle_limit;
    ...
    range = 2*filter_size/GPU_division_num;
    pad_num = ex_Nx/2 - filter_size;

else %상이 회절각 대비 가깝다
    ASM_filter = 0;
%     ex_map = zeros(ex_Nx,ex_Nx,'single');
...
for vv_cnt = 1:GPU_division_num
    ...
    u = (single(ex_dfx*[uu_range])); 
    v = (single(ex_dfx*[vv_range]));
...
kz_mat = real(sqrt(n_material.^2./lambda^2-repmat(u,size(v.')).^2 - repmat(v',size(u)).^2));          
for z_cnt = 1:1:z_num    
...
%  H_z = exp(- j * 2 * pi * real(sqrt(complex(n_material.^2./lambda^2-repmat(u,size(v.')).^2 - repmat(v',size(u)).^2))) * (-z_value)).*(sqrt(repmat(u,size(v.')).^2+repmat(v',size(u)).^2) < sind(theta_limit)/lambda);
 ...
 H_z = gpuArray(exp(-j*2*pi*(kz_mat)*(-z_value)).*(sqrt(repmat(u,size(v.')).^2+repmat(v',size(u)).^2) < sind(theta_limit)/lambda));   
...
division_list = [2 4 8 16 32 64 128 256 512 1024];
div_num = division_list(find(division_list >= div_num,1));
...
% temp = fftshift(ifft(ifftshift(padarray(gpuArray(m.ex_map(:,Nx_div*(cnt1-1)+1:Nx_div*cnt1)),[pad_num 0]),1)),1);
% m2.complex_temp(1:size(temp(y_range,:),1),Nx_div*(cnt1-1)+1:Nx_div*cnt1) = gather(temp(y_range,:));
...
decision_factor = GPU_max_Nx;
div_num = 2^ceil(log2((2*filter_size)^2/(decision_factor*1.2)^2));
division_list = [2 4 8 16 20 40 80 100 200 400 800];
div_num = division_list(find(division_list >= div_num,1));
