function [Epsr_PML Apsr_PML] = PRCWA_Gen_PML2D_only_y(pml_width,pml_N,n_surr,Nx,Ny,gambda_x,gambda_y)
...
%             Apsr_PML(mm,nn) = 1/epra*rect_2D(m,n,1,gambda_x,gambda_y,-pml_wx,+pml_wx,-pml_wy,+pml_wy);
             Epsr_PML(mm,nn) =   epra*rect_2D(m,n,1,gambda_x,gambda_y,-gambda_x/2,+gambda_x/2,-pml_wy,+pml_wy);
            Apsr_PML(mm,nn) = 1/epra*rect_2D(m,n,1,gambda_x,gambda_y, -gambda_x/2,+gambda_x/2,-pml_wy,+pml_wy);           
    end % for nn
end % for mm
...
for k = 1:pml_N
    ...
    for mm = 1:Nx
        for nn = 1:Ny
            m = mm-NBx;
            n = nn-NBy;
            t =  rect_2D(m,n,1,gambda_x,gambda_y,-gambda_x/2,+gambda_x/2,-pml2_wy,+pml2_wy) ...
                -rect_2D(m,n,1,gambda_x,gambda_y,-gambda_x/2,+gambda_x/2,-pml1_wy,+pml1_wy);
            Epsr_PML(mm,nn) = Epsr_PML(mm,nn) +   pml_epsr(k) * t;
            Apsr_PML(mm,nn) = Apsr_PML(mm,nn) + 1/pml_epsr(k) * t;
        end
    end
%     Fourier_coeff_2D_show(Epsr_PML,11,Nx,Ny,gambda_x,gambda_y);
%     k
%     pause(0.2);
end
