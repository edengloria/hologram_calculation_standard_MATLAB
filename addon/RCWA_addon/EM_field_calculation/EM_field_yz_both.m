function [Ex Ey Ez Hx Hy Hz] = field3D_on_yz_E_H_general(W,V,Q,invE,Cgp,Cgn,Kx,Ky,k0,x_1,y,z,acc_thick,width_type,dc_profile)
global L;
% USAGE
...
Ex          = zeros(zNum,yNum); % output region
Ey          = zeros(zNum,yNum);
Ez          = zeros(zNum,yNum);
Hx          = zeros(zNum,yNum);
Hy          = zeros(zNum,yNum);
...
%     Sz_     = iE * (Kx .* Sx_);
    Sx      = repmat(Sx_,1,yNum);
    Sy      = repmat(Sy_,1,yNum);
    Sz      = repmat(Sz_,1,yNum);
    Ux      = repmat(Ux_,1,yNum);
    Uy      = repmat(Uy_,1,yNum);
    Uz      = repmat(Uz_,1,yNum);
